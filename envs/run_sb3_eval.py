...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_hdrnet_cnn", type=str2bool, default=True)
    ...
    parser.add_argument("--net_arch", type=str, default="ultra", choices=["small", "big", "large", "default", "plus", "max", "ultra"])
    ...
    wandb_config = {"isp": args.isp,
                    ...
                    # "wb_param_dist_mode": args.wb_param_dist_mode,
    ...
    args.save_path = args.data_name + '-' + args.save_path + "-eval"
    # sub_name = args.save_path.split('-')
    # sub_name = '-'.join(sub_name[:-1])
    # args.save_path = sub_name + "/" + args.save_path
    ...
    if args.isp == "wb":
        ...
        elif dataset_name.startswith("fivek") or dataset_name.startswith("random"):
    ...
    rew, std_rew = evaluate_policy(model, env=eval_env, n_eval_episodes=len(eval_env.dataset), deterministic=True, render=False, )
