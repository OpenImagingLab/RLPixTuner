...
class Net(nn.Module):
    ...
    def loss_adain(self, output, content, style):
    ...
    def loss_gram(self,output,content,style):
        style_feats = self.encode_with_intermediate(style)
        content_feat = self.encode(content)
    ...
    def style_loss_adain(self, output, style):
        style_feats = self.encode_with_intermediate(style)
        out_feats = self.encode_with_intermediate(output)
        loss_s = self.calc_style_loss(out_feats[0], style_feats[0])
        for i in range(1, 4):
    ...
    def style_loss_gram(self, output, style):
        style_feats = self.encode_with_intermediate(style)
        out_feats = self.encode_with_intermediate(output)
        loss_s = 0.0
        for i in range(4):
    ...
    def content_loss(self, output, content):
        content_feat = self.encode(content)
        out_feat = self.encode(output)
        return self.calc_content_loss(out_feat, content_feat)

    def content_loss_layers(self, output, content):
        content_feats = self.encode_with_intermediate(content)
        out_feats = self.encode_with_intermediate(output)
        loss_c = self.calc_content_loss(out_feats[0], content_feats[0])
        for i in range(1, 4):
            loss_c += self.calc_content_loss(out_feats[i], content_feats[i])
        return loss_c